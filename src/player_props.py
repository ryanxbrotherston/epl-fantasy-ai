...
def player_rate_per90(roll_expected_stat: float, roll_minutes: float, fallback_roll_stat: float) -> float:
    ...
    return (rate_source or 0.0) / roll_minutes * 90
...
def player_fixture_props(player_row: pd.Series, team_lambda: float, league_avg_lambda: float,
                          expected_minutes: float = 75) -> dict:
    """player_row needs: roll_expected_goals, roll_expected_assists, roll_minutes,
    roll_goals_scored, roll_assists (all already-computed rolling features,
    e.g. from build_gw1_features.py's output or a live in-season equivalent)."""
    goal_rate90 = player_rate_per90(
        player_row.get("roll_expected_goals"), player_row.get("roll_minutes"), player_row.get("roll_goals_scored")
    )
    assist_rate90 = player_rate_per90(
        player_row.get("roll_expected_assists"), player_row.get("roll_minutes"), player_row.get("roll_assists")
    )
